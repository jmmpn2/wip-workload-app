...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${styles[status] || styles.normal}`}>
...
function LoadBar({
  ...
  return (
    <Link
      href={`/tech/${encodeURIComponent(technician)}`}
      className="block min-w-[150px] rounded-lg p-1 hover:bg-slate-50"
      title={`Open ${technician}'s detail page`}
    >
      <div className="mb-1 flex items-center justify-between">
        <span className={`text-sm font-semibold ${textColor}`}>{loadPct}%</span>
      </div>
            ...
            {techRank.map((row) => (
              <tr key={row.technician} className={`border-b border-slate-100 ${rowClasses(row.status)}`}>
                ...
                <td className="px-3 py-2 font-medium text-slate-900">
                  <Link href={`/tech/${encodeURIComponent(row.technician)}`} className="text-blue-700 hover:underline">
                    {row.technician}
                  </Link>
                </td>
                ...
                <td className="px-3 py-2">
                  <LoadBar loadPct={row.loadPct} status={row.status} technician={row.technician} />
                </td>
