...
export function StageSettingsForm({ stages }: { stages: StageRow[] }) {
  ...
  async function save() {
    setStatus("Saving...");
    const res = await fetch("/api/settings/stages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ stages: rows }) });
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-600">Each phase is its own stage in the dashboard. Adjust Remaining Hours logic here.</p>
        ...
        <table className="min-w-full border-collapse text-sm">
          ...
          <tbody>{rows.map((row, index) => (
            <tr key={row.id} className="border-b border-slate-100">
              ...
              <td className="px-3 py-2">{row.logicType === "FIXED" ? <input type="number" step="1" value={row.fixedHours ?? ""} onChange={(e) => { const copy=[...rows]; copy[index].fixedHours=e.target.value === "" ? null : Number(e.target.value); setRows(copy); }} className="w-28 rounded-lg border border-slate-300 px-3 py-2" /> : <input type="number" step="0.01" value={row.remainingPct} onChange={(e) => { const copy=[...rows]; copy[index].remainingPct=Number(e.target.value); setRows(copy); }} className="w-28 rounded-lg border border-slate-300 px-3 py-2" />}</td>
