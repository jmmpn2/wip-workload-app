...
export function CapacitySettingsForm({ technicians }: { technicians: { id: string; name: string; capacity: number; isActive: boolean }[] }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-600">Technician names come from the import. Weekly capacity is editable here.</p>
      <div className="mt-4 space-y-3">{rows.map((row, index) => (
        <div key={row.id} className="grid gap-3 rounded-xl border border-slate-200 p-3 md:grid-cols-[1fr_160px_120px]">
