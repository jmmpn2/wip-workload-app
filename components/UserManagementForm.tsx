...
export function UserManagementForm({ users, shops, allowedRoles, canManageAllShops }: { users: UserRow[]; shops: { id: string; name: string }[]; allowedRoles: Role[]; canManageAllShops: boolean; }) {
  ...
  async function createUser(e: React.FormEvent) {
    ...
    const res = await fetch("/api/users", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, role, shopId: shopId || null, tempPassword }) });
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={createUser} className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:grid-cols-4">
        <div>
