...
function HandoutTable({ title, description, rows, variant, canMoveJobs, canEditNotes }: { title: string; description: string; rows: BucketRow[]; variant: "assignable" | "tow_in" | "hold"; canMoveJobs: boolean; canEditNotes: boolean; }) {
  ...
  function toggleSort(nextKey: SortKey) {
    if (nextKey === sortKey) return setDirection(direction === "asc" ? "desc" : "asc");
    setSortKey(nextKey);
    ...
    <div id={sectionId} className="scroll-mt-24 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 max-h-[520px] overflow-y-auto overflow-x-visible">
        <table className="w-full table-fixed border-collapse text-sm">
          <thead className="sticky top-0 bg-white"><tr className="border-b border-slate-200 text-slate-600">
            ...
            {canMoveJobs ? <th className="w-[170px] px-3 py-2 print-hidden">Action</th> : null}
          </tr></thead>
          <tbody>
            {sortedRows.length ? sortedRows.map((row) => (
              <tr key={row.id} className={`border-b border-slate-100 ${row.isHighlightedInsurance ? "bg-sky-50/80 ring-1 ring-inset ring-sky-200" : variant === "hold" ? "bg-amber-50" : variant === "tow_in" ? "bg-slate-50" : ""}`}>
                <td className={`px-3 py-2 ${row.isHighlightedInsurance ? "border-l-4 border-sky-500 font-semibold" : ""}`}><div>{row.roNumber}</div>{row.isHighlightedInsurance ? <div className="mt-1 text-[10px] font-semibold uppercase tracking-wide text-sky-700">★ Focus insurer</div> : null}</td>
                <td className="px-3 py-2">{row.owner}</td><td className="px-3 py-2">{row.vehicle}</td><td className="px-3 py-2">{row.estimator || "—"}</td><td className="px-3 py-2">{row.isHighlightedInsurance ? <div><div className="font-semibold text-slate-900">{row.insurance || "—"}</div><div className="mt-1 text-[10px] font-semibold uppercase tracking-wide text-sky-700">Focus insurer</div></div> : (row.insurance || "—")}</td><td className="px-3 py-2 text-right">{Math.round(row.daysInShop || 0)}</td><td className="px-3 py-2">{row.stage}</td><td className="px-3 py-2 text-right">{Math.round(row.roHours)}</td><td className="px-3 py-2 text-right">{Math.round(row.remainingHours)}</td>
                <td className="px-3 py-2">{canEditNotes ? <JobBucketNoteEditor roNumber={row.roNumber} initialNote={row.handoutNote || ""} initialLastEditedByName={row.noteLastEditedByName || null} initialLastEditedAt={row.noteLastEditedAt || null} /> : <div title={row.noteLastEditedByName && row.noteLastEditedAt ? `Last edited by ${row.noteLastEditedByName} on ${formatCentralDateTime(row.noteLastEditedAt)}` : undefined} className="w-[190px] min-w-[190px] rounded-lg border border-slate-200 bg-slate-50 px-2 py-2 text-xs text-slate-700 whitespace-pre-wrap">{row.handoutNote || "—"}</div>}</td>
                <td className="px-3 py-2">{variant === "hold" ? <div className="text-amber-800"><div className="font-semibold">ON HOLD</div>{row.handoutHoldReason ? <div className="text-xs">{row.handoutHoldReason}</div> : null}</div> : variant === "tow_in" ? <div className="font-semibold text-slate-600">Tow in - Needs Estimate</div> : <span className="text-slate-400">Ready to handout</span>}</td>
            ...
            )) : <tr><td colSpan={canMoveJobs ? 12 : 11} className="px-3 py-8 text-center text-slate-500">No cars in this section.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
export function AssignableJobsTable({ rows, towInRows, holdRows, canMoveJobs, canEditNotes }: { rows: BucketRow[]; towInRows: BucketRow[]; holdRows: BucketRow[]; canMoveJobs: boolean; canEditNotes: boolean; }) {
  return <div className="space-y-6"><HandoutTable title="Cars To Handout" description="These jobs have no technician assigned and are available to hand out." rows={rows} variant="assignable" canMoveJobs={canMoveJobs} canEditNotes={canEditNotes} /><HandoutTable title="Tow Ins Needing Estimate" description="These unassigned tow-ins have been tagged as needing an estimate before they are ready to hand out." rows={towInRows} variant="tow_in" canMoveJobs={canMoveJobs} canEditNotes={canEditNotes} /><HandoutTable title="Cars On Hold" description="These jobs are not ready to hand out yet. Release them here to move them back to Cars To Handout." rows={holdRows} variant="hold" canMoveJobs={canMoveJobs} canEditNotes={canEditNotes} /></div>;
}
