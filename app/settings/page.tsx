...
export default async function SettingsPage() {
  await requireRoleAccess({ settings: true });
  ...
  return (
    <div className="space-y-6">
      ...
      <HighlightedInsuranceSettingsForm highlightedInsurers={highlightedInsurers.map((row) => row.insuranceName)} availableInsurers={[...new Set(currentRows.map((row) => row.insurance))].sort((a, b) => a.localeCompare(b))} />
