      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm print:rounded-none print:border-none print:p-0 print:shadow-none">
        ...
        <div className="mt-3 overflow-x-auto">
          <table className="min-w-full border-collapse text-sm print:text-[11px]">
            ...
            <tbody>
              {data.rows.map((row) => (
                <tr key={row.id} className={`border-b border-slate-100 ${row.isHeld ? "bg-amber-50" : ""}`}>
                  <td className="px-3 py-2 print:px-2 print:py-1.5">{row.roNumber}</td><td className="px-3 py-2 print:px-2 print:py-1.5">{row.owner}</td><td className="px-3 py-2 print:px-2 print:py-1.5">{row.vehicle}</td><td className="px-3 py-2 print:px-2 print:py-1.5">{row.estimator || "—"}</td><td className="px-3 py-2 print:px-2 print:py-1.5">{row.insurance || "—"}</td><td className="px-3 py-2 print:px-2 print:py-1.5">{Math.round(row.daysInShop || 0)}</td><td className="px-3 py-2 print:px-2 print:py-1.5">{row.stage}</td><td className="px-3 py-2 print:px-2 print:py-1.5">{Math.round(row.roHours)}</td><td className="px-3 py-2 print:px-2 print:py-1.5">{Math.round(row.remainingHours)}</td>
