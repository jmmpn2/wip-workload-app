...
export default async function PrintAllTechReportsPage() {
  ...
  const techDetails = await Promise.all(
    dashboard.techRank.map((tech) => getTechDetail(shopId, tech.technician))
  );
    ...
    <div className="space-y-6 print:space-y-0">
      ...
      {techDetails.map((data, index) => {
        ...
        return (
          <section
            key={technicianName}
            className="tech-report-page rounded-2xl border border-slate-200 bg-white p-5 shadow-sm print:mb-0 print:rounded-none print:border-none print:p-0 print:shadow-none print:break-after-page"
          >
                  ...
                  <tbody>
                    {data.techRank.map((row) => {
                      ...
                      return (
                        ...
                        >
                          <td className={`px-3 ${isCurrentTech ? "py-3 print:py-2" : "py-2 print:py-1.5"}`}>{row.rank}</td>
                          <td className={`px-3 ${isCurrentTech ? "py-3 print:py-2" : "py-2 print:py-1.5"}`}>{row.technician}</td>
                          <td className={`px-3 ${isCurrentTech ? "py-3 print:py-2" : "py-2 print:py-1.5"}`}>{row.remainingHours}</td>
                          <td className={`px-3 ${isCurrentTech ? "py-3 print:py-2" : "py-2 print:py-1.5"}`}>{row.loadPct}%</td>
                        </tr>
                      );
                    })}
                  </tbody>
                        ...
                        <td className="px-3 py-2 print:px-2 print:py-1.5">
                          {row.isHeld ? (
                            <div className="text-amber-800">
                              ...
                              {row.holdReason ? <div className="text-xs">{row.holdReason}</div> : null}
                            </div>
                          ) : (
