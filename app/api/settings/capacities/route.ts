...
export async function POST(request: NextRequest) {
  ...
  await logShopAudit({
    shopId,
    action: "UPDATE_CAPACITIES",
    entityType: "SETTINGS",
    summary: `Updated capacity settings for ${technicians.length} technician${technicians.length === 1 ? "" : "s"}.`,
    metadata: { technicianCount: technicians.length },
  });
